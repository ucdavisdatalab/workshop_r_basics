...
columns by name. For instance, let's get the `name`, `profession`, and `currency`
columns for all women on banknotes:

```{r}
result = banknotes[banknotes$gender == "F", c("name", "profession", "currency")]
head(result)
```
...
As an example, let's plot the banknotes data. First, we need to load ggplot2. As
...
show. Let's make a line plot that shows mean year of death of people who first
appeared on currency since 2012, with separate lines for men and women.

Before plotting, we need to take a subset of the banknotes that only contains
information since 2012:

```{r}
notes12 = banknotes[banknotes$first_appearance_year >= 2012, ]
```
...
```{r}
notes12 = notes12[notes12$race == "All Races" &
  notes12$ethnic_origin == "All Origins" &
  notes12$age == "16 years and over", ]
```
