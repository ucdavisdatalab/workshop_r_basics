...
explore a data set about the people who are depicted on paper money from around
the world. [Click here to download the data set][banknotes-dl] (you may need to
...
that this data set has eight columns. A more convenient way to check the number
...
column, and `banknotes$death_year` selects the `death_year` column. So one way
to compute the mean of the `death_year` column is:
