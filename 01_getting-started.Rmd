...
called _arguments_, the output is called the _return value_, and when we use a
function, we say we're _calling_ the function.
...
explore data from The Pudding about the people who are depicted on paper money
around the world. You can find more details about the data set [here][banknotes], and you can
download the data set [here][banknotes-dl] (you may need to choose `File -> Save
...
The data set is a file called `banknotes.csv`, which suggests it's a CSV file. In
...
By counting the columns in the output from `head( banknotes )`, we can see that this
...
For instance, for the banknotes data, `banknotes$country` selects
the `country` column, and `banknotes$death_year` selects the `death_year` column. So one
way to compute the mean of the `death_year` column is:

```{r}
mean( banknotes$death_year )
```

Similarly, to compute the range of the `value` column:

```{r}
range( banknotes$value )
```

You can also use the dollar sign operator to assign values to columns. For
instance, to assign `USD` to the entire `abbr` column:
...
applied any transformations to the banknotes data yet, so we can reset the
`banknotes` variable back to what it was by reloading the data set:
